...
import json
import os
from PyQt5.QtWidgets import QTableWidgetItem, QFileDialog, QMessageBox, QWidget
...
def calcular_juros(self):
    ...
    try:
        ...
        self.graficos_meses = meses_plot
        self.graficos_saldos = saldos
        self.graficos_juros = juros_acumulados
        self.graficos_total_investido = total_investido
        self.graficos_total_juros = total_juros
...
def limpar_campos(self):
    """Limpa campos, tabela e gráficos apenas se houver cálculo."""
    try:
        if not hasattr(self, "tabela") or self.tabela.rowCount() == 0:
            QMessageBox.warning(self, "Limpeza bloqueada",
                                "Não há dados calculados para limpar.")
            return

        # Limpa campos
        for campo in ["capital", "taxa", "tempo", "capital_inicial", "aporte_mensal", "taxa_juros"]:
            if hasattr(self, campo):
                getattr(self, campo).clear()

        if hasattr(self, "tipo_taxa"):
            self.tipo_taxa.setCurrentIndex(0)
        if hasattr(self, "tipo_tempo"):
            self.tipo_tempo.setCurrentIndex(0)

        # Zera totais e tabela
        if hasattr(self, "tabela"):
            self.tabela.setRowCount(0)
        if hasattr(self, "total_juros"):
            self.total_juros.setText("R$ 0.00")
        if hasattr(self, "total_final"):
            self.total_final.setText("R$ 0.00")

        # Zera gráficos
        self.graficos_meses, self.graficos_saldos, self.graficos_juros = [], [], []

        # Remove widgets antigos do layout de gráficos
        if hasattr(self, "layout_graficos"):
            for i in reversed(range(self.layout_graficos.count())):
                widget = self.layout_graficos.itemAt(i).widget()
                if widget:
                    widget.setParent(None)

        # Recria gráficos vazios (mantém layout visual)
...
def validar_e_calcular(self):
    """Valida se os campos obrigatórios estão preenchidos antes de calcular."""
    try:
        campos_obrigatorios = []

        # Juros Simples
        if hasattr(self, "capital"):
            campos_obrigatorios.append(self.capital.text().strip())
        if hasattr(self, "taxa"):
            campos_obrigatorios.append(self.taxa.text().strip())
        if hasattr(self, "tempo"):
            campos_obrigatorios.append(self.tempo.text().strip())

        # Juros Compostos / Primeiro Milhão
        if hasattr(self, "capital_inicial"):
            campos_obrigatorios.append(self.capital_inicial.text().strip())
        if hasattr(self, "aporte_mensal"):
            campos_obrigatorios.append(self.aporte_mensal.text().strip())
        if hasattr(self, "taxa_juros"):
            campos_obrigatorios.append(self.taxa_juros.text().strip())

        # Se algum campo estiver vazio, bloqueia o cálculo
        if any(c == "" for c in campos_obrigatorios):
            QMessageBox.warning(self, "Cálculo bloqueado",
                                "Preencha todos os campos antes de calcular.")
            return

        # Se todos os campos estão preenchidos, chama o cálculo normalmente
        self.calcular()

    except Exception as e:
        QMessageBox.warning(self, "Erro", f"Não foi possível validar os dados:\n{e}")
    ...
    try:
        ...
        if not hasattr(self, "tabela") or self.tabela.rowCount() == 0:
            ...
            if parent_widget is not None:
                QMessageBox.warning(parent_widget, "Exportação bloqueada", 
                                    "Não há dados calculados para exportar.")
            else:
                print("Exportação bloqueada: não há dados calculados.")
            return  # sai da função sem exportar
        ...
        filename, filtro = QFileDialog.getSaveFileName(
            parent,
            "Salvar exportação",
            os.path.join(pasta_downloads, "dados_exportados.csv"),
        ...
        if parent_widget is not None:
            QMessageBox.information(parent_widget, "Exportação concluída", f"Dados exportados para:\n{filename}")
        else:
            print(f"Exportação concluída: {filename}")

    except Exception as e:
        if parent_widget is not None:
            QMessageBox.warning(parent_widget, "Erro ao exportar", f"Não foi possível exportar os dados:\n{e}")
        else:
