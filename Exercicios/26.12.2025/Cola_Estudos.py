...
print(df[df["nota"] > 9])                # alunos com nota acima de 9
    ...
    # Filtro com duas condições (E lógico)
    filtro = df[(df["Idade"] > 20) & (df["Cidade"] == "São Paulo")]
    print(df[filtro])
    
    # Filtro com duas condições (OU lógico)
    filtro2 = df[(df["Idade"] > 20) | (df["Cidade"] == "São Paulo")]
    print(df[filtro2])
