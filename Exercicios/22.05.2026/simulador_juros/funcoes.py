...
def calcular_juros(self):
    ...
    try:
        ...
        for mes in range(1, tempo + 1):
            ...
            self.tabela.setItem(self.tabela.rowCount()-1, 1, criar_item(format_currency(saldo)))
            self.tabela.setItem(self.tabela.rowCount()-1, 2, criar_item(format_currency(base_calculo)))
            self.tabela.setItem(self.tabela.rowCount()-1, 3, criar_item(format_currency(rendimento_bruto)))
            self.tabela.setItem(self.tabela.rowCount()-1, 4, criar_item(f"{aliquota_ir:.1f}%"))
            self.tabela.setItem(self.tabela.rowCount()-1, 5, criar_item(format_currency(rendimento_liquido)))
            self.tabela.setItem(self.tabela.rowCount()-1, 6, criar_item(format_currency(saldo_final)))
...
def exportar_dados(self):
    ...
    try:
        ...
        filename, filtro = QFileDialog.getSaveFileName(
            ...
            "CSV (*.csv);;JSON (*.json);;Excel (*.xls);;PDF (*.pdf);;Todos os arquivos (*)",
            options=options
        )
        if not filename:
            return
        ...
        selected_filter = filtro.lower() if filtro else ""
        ext = os.path.splitext(filename)[1].lower()

        if "csv" in selected_filter or ext == ".csv":
            if ext != ".csv":
                filename += ".csv"
            _save_csv(filename, headers, rows)
        elif "json" in selected_filter or ext == ".json":
            if ext != ".json":
                filename += ".json"
            _save_json(filename, headers, rows)
        elif "excel" in selected_filter or ext in [".xls", ".xlsx"]:
            if ext not in [".xls", ".xlsx"]:
                filename += ".xls"
            _save_excel_html(filename, headers, rows)
        elif "pdf" in selected_filter or ext == ".pdf":
            if ext != ".pdf":
                filename += ".pdf"
            _save_pdf(self, filename, headers, rows)
        else:
            if ext != ".csv":
                filename += ".csv"
            _save_csv(filename, headers, rows)
