...
class ControleGastos(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.input_salario.returnPressed.connect(self.atualizar_salario)
        ...
        self.input_categoria.returnPressed.connect(self.adicionar_gasto)
        self.input_valor.returnPressed.connect(self.adicionar_gasto)
        ...
        btn_salvar_gastos = QPushButton("💾 Salvar Gastos")
        btn_salvar_gastos.setFixedSize(140, 35)
        btn_salvar_gastos.clicked.connect(self.salvar_gastos)
        self._style_button(btn_salvar_gastos)
        botoes_layout.addWidget(btn_salvar_gastos)

        btn_carregar_gastos = QPushButton("📥 Carregar Gastos")
        btn_carregar_gastos.setFixedSize(140, 35)
        btn_carregar_gastos.clicked.connect(self.carregar_gastos)
        self._style_button(btn_carregar_gastos)
        botoes_layout.addWidget(btn_carregar_gastos)

        btn_ver_grafico = QPushButton("📊 Ver Gráfico de Gastos Mensais")
        btn_ver_grafico.setFixedSize(200, 35)
        ...
        self.input_banco.returnPressed.connect(self.adicionar_banco)
        self.input_saldo.returnPressed.connect(self.adicionar_banco)
        ...
        btn_salvar_bancos = QPushButton("💾 Salvar Bancos")
        btn_salvar_bancos.setFixedSize(140, 35)
        btn_salvar_bancos.clicked.connect(self.salvar_bancos)
        self._style_button(btn_salvar_bancos)

        btn_carregar_bancos = QPushButton("📥 Carregar Bancos")
        btn_carregar_bancos.setFixedSize(140, 35)
        btn_carregar_bancos.clicked.connect(self.carregar_bancos)
        self._style_button(btn_carregar_bancos)
        ...
        botoes_bancos_layout.addWidget(btn_salvar_bancos)
        botoes_bancos_layout.addWidget(btn_carregar_bancos)
    ...
    def salvar_gastos(self):
        ...
        for row in range(self.table_gastos.rowCount()):
            ...
            if categoria_item and valor_item:
                ...
                valor = valor_item.text()
                dados.append({"categoria": categoria, "valor": valor})
    ...
    def carregar_gastos(self):
        ...
        try:
            ...
            if not isinstance(dados, list):
                raise ValueError("Arquivo JSON inválido para gastos")
            for item in dados:
                if not self._validar_registro_gasto(item):
                    raise ValueError("Arquivo JSON não corresponde a gastos mensais")
            ...
            for item in dados:
                ...
                valor_texto = item.get("valor", "R$ 0,00")
                try:
                ...
                item_val = QTableWidgetItem(valor_texto)
                ...
                btn_up.clicked.connect(
                    lambda _, r=row: self.mover_gasto_cima(r)
                )
                ...
                btn_down.clicked.connect(
                    lambda _, r=row: self.mover_gasto_baixo(r)
                )
                ...
                btn_edit.clicked.connect(
                    lambda _, r=row: self.editar_gasto_por_botao(r)
                )
                ...
                btn_remove.clicked.connect(
                    lambda _, r=row: self.remover_gasto(r)
                )
    ...
    def _validar_registro_gasto(self, item):
        ...
        if not isinstance(valor, str) or not valor.strip():
            return False
        try:
            self._parse_monetary(valor)
        except ValueError:
            return False
        return True

    def _validar_registro_banco(self, item):
        if not isinstance(item, dict):
            return False
        ...
        if not isinstance(banco, str) or not banco.strip():
            return False
        if not isinstance(saldo, str) or not saldo.strip():
            return False
        try:
            self._parse_monetary(saldo)
        except ValueError:
            return False
        return True
    ...
    def salvar_bancos(self):
        ...
        for row in range(self.table_bancos.rowCount()):
            ...
            if banco_item and saldo_item:
                ...
                saldo = saldo_item.text()
                dados.append({"banco": banco, "saldo": saldo})

        try:
            with open(caminho, "w", encoding="utf-8") as f:
                json.dump(dados, f, ensure_ascii=False, indent=2)
            QMessageBox.information(self, "Salvar Bancos", f"Bancos salvos em:\n{caminho}")
        except Exception as e:
            QMessageBox.warning(self, "Erro ao salvar", f"Não foi possível salvar o arquivo:\n{e}")
    ...
    def carregar_bancos(self):
        ...
        try:
            ...
            if not isinstance(dados, list):
                raise ValueError("Arquivo JSON inválido para bancos")
            for item in dados:
                if not self._validar_registro_banco(item):
                    raise ValueError("Arquivo JSON não corresponde a valores em bancos")

            self.table_bancos.blockSignals(True)
            self.table_bancos.setRowCount(0)
            self.bancos = []
            for item in dados:
                banco = item.get("banco", "")
                saldo_texto = item.get("saldo", "R$ 0,00")
                try:
                ...
                item_saldo = QTableWidgetItem(saldo_texto)
                ...
                btn_up.clicked.connect(
                    lambda _, r=row: self.mover_banco_cima(r)
                )
                ...
                btn_down.clicked.connect(
                    lambda _, r=row: self.mover_banco_baixo(r)
                )
                ...
                btn_edit.clicked.connect(
                    lambda _, r=row: self.editar_banco_por_botao(r)
                )
                ...
                btn_remove.clicked.connect(
                    lambda _, r=row: self.remover_banco(r)
                )
            ...
            self.atualizar_resumo()
            QMessageBox.information(self, "Carregar Bancos", "Bancos carregados com sucesso.")
        except Exception as e:
            QMessageBox.warning(self, "Erro ao carregar", f"Não foi possível carregar o arquivo:\n{e}")
    ...
    def adicionar_gasto(self):
        ...
        btn_up.setToolTip("Mover gasto para cima")
        btn_up.clicked.connect(
            lambda _, w=actions_widget: self.mover_gasto_cima(
                self._find_row_by_cell_widget(self.table_gastos, w)
            )
        )
        ...
        btn_down.setToolTip("Mover gasto para baixo")
        btn_down.clicked.connect(
            lambda _, w=actions_widget: self.mover_gasto_baixo(
                self._find_row_by_cell_widget(self.table_gastos, w)
            )
        )
        ...
        btn_edit.setToolTip("Editar valor do gasto")
        btn_edit.clicked.connect(
            lambda _, w=actions_widget: self.editar_gasto_por_botao(
                self._find_row_by_cell_widget(self.table_gastos, w)
            )
        )
        ...
        btn_remove.setToolTip("Remover gasto")
        btn_remove.clicked.connect(
            lambda _, w=actions_widget: self.remover_gasto(
                self._find_row_by_cell_widget(self.table_gastos, w)
            )
        )
    ...
    def _find_row_by_cell_widget(self, table, widget):
        for row in range(table.rowCount()):
            if table.cellWidget(row, 2) is widget:
                return row
        return -1
    ...
    def adicionar_banco(self):
        ...
        btn_up.setToolTip("Mover banco para cima")
        btn_up.clicked.connect(
            lambda _, w=actions_widget: self.mover_banco_cima(
                self._find_row_by_cell_widget(self.table_bancos, w)
            )
        )
        ...
        btn_down.setToolTip("Mover banco para baixo")
        btn_down.clicked.connect(
            lambda _, w=actions_widget: self.mover_banco_baixo(
                self._find_row_by_cell_widget(self.table_bancos, w)
            )
        )
        ...
        btn_edit.setToolTip("Editar saldo do banco")
        btn_edit.clicked.connect(
            lambda _, w=actions_widget: self.editar_banco_por_botao(
                self._find_row_by_cell_widget(self.table_bancos, w)
            )
        )
        ...
        btn_remove.setToolTip("Remover banco")
        btn_remove.clicked.connect(
            lambda _, w=actions_widget: self.remover_banco(
                self._find_row_by_cell_widget(self.table_bancos, w)
            )
        )
    ...
    def mostrar_grafico(self):
        ...
        text_color = "#F9FAFB" if self.dark_mode else "#111827"
        face_color = "#111827" if self.dark_mode else "#FFFFFF"
        fig_face = "#1F2937" if self.dark_mode else "#F3F4F6"
        ...
        self.ax.set_title("Distribuição do Salário", fontsize=12, fontweight="bold", color=text_color)
        ...
        legend.get_frame().set_facecolor(fig_face)
    ...
    def mostrar_grafico_bancos(self):
        ...
        self.ax.set_title("Distribuição dos Saldos em Bancos", fontsize=12, fontweight="bold", color=text_color)
        for txt in texts + autotexts:
            txt.set_color(text_color)
