...
from core.funcoes import exportar_dados, format_currency
...
class JurosSimples(QWidget):
    ...
    def initUI(self):
        ...
        self.taxa.setPlaceholderText("Ex.: 1.05 %")
        self.taxa.editingFinished.connect(self._formatar_taxa)
    ...
    def calcular(self):
        ...
        try:
            ...
            grafico2 = grafico_composicao(capital, total_juros, dark_mode=self.dark_mode)
    ...
    def _formatar_taxa(self):
        ...
        try:
            if ',' in s or '.' in s:
                valor = self._ler_valor(self.taxa)
            else:
                if s.isdigit():
                    if len(s) >= 3:
                        valor = int(s) / 100.0
