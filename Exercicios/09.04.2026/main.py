...
def opcoes(nome_conta):
    conexao, cursor = conexao_cursor()

    while True:
        try:
            cursor.execute(
                "SELECT senha_conta FROM conta_bancaria WHERE nome_conta = ?",
                (nome_conta,)
            )
            resultado = cursor.fetchone()
            ...
            if resultado:
                ...
                print(f'Sua senha atual é: {senha_atual}')
                ...
                for i, opcao in enumerate(listar_opcoes):
                    print(f'{i} - {opcao}')
                ...
                match escolha:
                    ...
                    case 1:
                        ...
                        if nova_senha == senha_atual:
                            print('Você não pode usar a mesma senha!')
                            continue
                        ...
                        cursor.execute(
                            "UPDATE conta_bancaria SET senha_conta = ? WHERE nome_conta = ?",
                            (nova_senha, nome_conta)
                        )
                        conexao.commit()
